            ...
            <div class="layout-px-spacing">
             <div class="chat-section layout-top-spacing">
              <div class="widget-content widget-content-area br-6">
               ...
               <h1>Pending for Approval Departure <span class="btn btn-info btn-sm pull-right">Total Pending <span style="color:#ffeb00">{{$pending}}</span></span></h1>
                 ...
                 @if(count($departures)> 0 )
                  @foreach( $departures as $key => $departure )
                    <tr>
                      ...
                      <td>@if($departure->start_date >= date('Y-m-d'))
                      {{date('d-M-Y', strtotime($departure->start_date))}}
                      @else
                      <span class="text-danger">Expaire</span>
                      @endif
                      </td>
                      ...
                      <td>
                        @if($departure->approve == '1')
                            <a class="disableDepartue badge badge-success text-light" data-id="{{ $departure->id }}" data-status="{{ $departure->approve }}" style="cursor: pointer; color: #2f8263;">Approved
                            </a>
                          @else
                          <a class="disableDepartue badge badge-danger text-light" data-id="{{ $departure->id }}" data-status="{{ $departure->approve }}" style="cursor: pointer; color: #F9423C;">
                             Pending
                          </a>
                          @endif
                      </td>
